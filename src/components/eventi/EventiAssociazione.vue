<script>
import { GetEventiAssociazioneEndpoint } from "@/endpoints";
...
export default {
  ...
  methods: {
    async getEventi() {
      fetch(`${GetEventiAssociazioneEndpoint}?id=${encodeURIComponent(this.id)}`, {
        method: "GET",
        headers: {
...
<template>
  <h2 class="text-2xl font-bold text-center section-spacing">Events</h2>
